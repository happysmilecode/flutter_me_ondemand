...
import 'package:socialv/screens/post/components/mentions_list_component.dart';
          ...
          SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                if (mentionsMemberList.isNotEmpty)
                  MentionsListComponent(
                    mentionsMemberList: mentionsMemberList,
                    callback: (mentionName) {
                      final int atSymbolIndex = postContentTextEditController.text.indexOf('@');
                      String text = postContentTextEditController.text;

                      if (atSymbolIndex != -1) {
                        final int spaceIndex = text.indexOf(' ', atSymbolIndex);
                        if (spaceIndex != -1) {
                          text = text.replaceAll(text.substring(atSymbolIndex + 1, spaceIndex), mentionName);
                        } else {
                          text = text.replaceAll(text.substring(atSymbolIndex + 1), mentionName);
                        }
                      }

                      mentionText = "";
                      mentionsMemberList.clear();
                      postContentTextEditController.text = text;
                      postContentTextEditController.selection = TextSelection(baseOffset: text.length, extentOffset: text.length);
                      setState(() {});
                    },
                  ),
                TextField(
                  ...
                  onChanged: (text) {
                    ...
                    if (atSymbolIndex != -1) {
                      final int spaceIndex = text.indexOf(' ', atSymbolIndex);
                      if (spaceIndex != -1) {
                        mentionText = text.substring(atSymbolIndex + 1, spaceIndex);
                        getMentionsMembers();
                      } else {
                        mentionText = text.substring(atSymbolIndex + 1);
                        getMentionsMembers();
                      }
                    } else {
                      /// normal texts
